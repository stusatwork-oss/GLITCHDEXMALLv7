...
objs=simp.obj pnd3d.obj drawcone.obj kplib.obj cwinmain.obj
libs=ole32.lib user32.lib gdi32.lib comdlg32.lib
!if 0
opts=/c /TP /Ox /Ob2 /G6Fy /Gs /MD /QIfist #VC6 only
libs=%(libs) /opt:nowin98                  #VC6 only
!else
opts=/c /TP /Ox /Ob2 /GFy /Gs /MT          #> VC6
!endif
simp.exe: $(objs) simp.c; link $(objs) $(libs) /nologo
	del simp.obj
simp.obj:     simp.c pnd3d.h cwinmain.h ; cl simp.c     $(opts)
pnd3d.obj:    pnd3d.c pnd3d.h cwinmain.h; cl pnd3d.c    $(opts)
drawcone.obj: drawcone.c                ; cl drawcone.c $(opts)
kplib.obj:    kplib.c                   ; cl kplib.c    $(opts)
cwinmain.obj: cwinmain.c cwinmain.h     ; cl cwinmain.c $(opts)
!if 0
#endif
...
int WINAPI WinMain (HINSTANCE hinst, HINSTANCE hpinst, LPSTR cmdline, int ncmdshow)
{
	...
	float f;
	int i, x, y, z, isurf, obstatus = 0;

	xres = 1024; yres = 768; prognam = "SIMP by Ken Silverman";
	...
	while (!breath())
	{
		...
		otim = tim; tim = klock(); dtim = tim-otim;
			...
			//Handle rotation
		if (!(bstatus&2)) { oct_rotvex((float)dmousx*.01,&ifor,&irig); f = irig.y*.1; } else f = (float)dmousx*-.01;
		oct_rotvex((float)dmousy*.01f,&ifor,&idow);

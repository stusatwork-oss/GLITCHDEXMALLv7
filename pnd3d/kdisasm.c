...
static const x86_t op_66_0f_38_xx[] =
{
	...
	0x30, GKVoWq  ,GKVoWd  ,GKVoWw  ,GKVoWq  ,GKVoWd  ,GKVoWq  ,GKVoWo  ,GKVoWo  , "pmovzxbw" ,"pmovzxbd"  ,"pmovzxbq" ,"pmovzxwd" ,"pmovzxwq" ,"pmovzxdq"  ,"permd"  ,"pcmpgtq"         ,
	0x38, GKVoWo  ,GKVoWo  ,GKVoWo  ,GKVoWo  ,GKVoWo  ,GKVoWo  ,GKVoWo  ,GKVoWo  , "pminsb"   ,"pminsd"    ,"pminuw"   ,"pminud"   ,"pmaxsb"   ,"pmaxsd"    ,"pmaxuw" ,"pmaxud"          ,
	0x40, GKVoWo  ,GKVoWo  ,GK      ,GK      ,GK      ,GKVoWo  ,GKVoWo  ,GKVoWo  , "pmulld"   ,"phminposuw",""         ,""         ,""         ,"psrlvd"    ,"psravd" ,"psllvd"          ,
...
enum
{
	...
	GOTVEX=(1<<21),GOTEVEX=(1<<22),GOT256=(1<<23),
	BITS64=(1<<24),
};
...
static int getadst (unsigned char *ist, char *ost, int siz, int prefix)
{
	...
	if ((unsigned char)ist[0] >= 0xc0)
	{
		...
		switch(siz)
		{
			...
			case -16: sprintf(ost,"%cmm%d",'x'+((prefix&GOT256)!=0),i); return(1);
		}
		return(0);
	}
...
{
	int i, j, x, y, prefix, regind, repind, xnum, opsize, opsizedef, mode, mode2, oi, rexreg;
	...
	for(i=0;i<4;i++) //x86 has up to 4 prefixes.
	{
		if (bits == 64)
		{
			...
			{
				prefix |= GOTVEX;
				if ((ibuf[i+1]&128) == 0) { prefix |= GOTREXR; }
				if ((ibuf[i+1]& 64) == 0) { prefix |= GOTREXX; }
				if ((ibuf[i+1]& 32) == 0) { prefix |= GOTREXB; }
				if ((ibuf[i+1]& 31) == 1) { prefix |= GOT0F; }
				if ((ibuf[i+1]& 31) == 2) { prefix |= GOT0F; prefix |= GOT38; }
				if ((ibuf[i+1]& 31) == 3) { prefix |= GOT0F; prefix |= GOT3A; }
				if ((ibuf[i+2]&128) != 0) { prefix |= GOTREXW; }
				rexreg = ((ibuf[i+2]>>3)&15)^15;
				if ((ibuf[i+2]&  4) != 0) { prefix |= GOT256; }
				if ((ibuf[i+2]&  3) == 1) { prefix |= GOT66; }
				if ((ibuf[i+2]&  3) == 2) { prefix |= GOTF3; }
				if ((ibuf[i+2]&  3) == 3) { prefix |= GOTF2; }
				i = 3; goto nomoreprefixes;
			}
			if (ibuf[i] == 0xc5)
			{
				prefix |= GOTVEX|GOT0F;
				if ((ibuf[i+1]&128) == 0) { prefix |= GOTREXR; }
				rexreg = ((ibuf[i+1]>>3)&15)^15;
				if ((ibuf[i+1]&  4) != 0) { prefix |= GOT256; }
				if ((ibuf[i+1]&  3) == 1) { prefix |= GOT66; }
				if ((ibuf[i+1]&  3) == 2) { prefix |= GOTF3; }
				if ((ibuf[i+1]&  3) == 3) { prefix |= GOTF2; }
				i = 2; goto nomoreprefixes;
			}
			if (ibuf[i] == 0x62) { prefix |= GOTEVEX; i = 1; goto nomoreprefixes;}
		}
		switch(ibuf[i])
		{
				//Prefix group 1:
...
nomoreprefixes:;
	...
	if (prefix&GOTVEX) //FUKFUK!
	{
		if (ibuf[i] == 0x77) { if (!(prefix&GOT256)) strcpy(obuf,"vzeroupper"); else strcpy(obuf,"vzeroall"); return(i+1); } //FUK:hack!
		if (prefix&GOT38)
		{
	...
	else if (ibuf[i] == 0x0f) //Handle 2/3 byte opcode escapes/extensions
	{
		i++; prefix |= GOT0F;
		if (ibuf[i] == 0x38)
		{
...
doagain:;
		...
		switch(mode)
		{
			...
			case GKAp: break;
			case GKFv: if ((opsize == 4) && (bits == 64)) opsize = 8; strcpy(&cptr[-1],wordptrst[opsize]); break; //FUK
			...
			case GKVoPRq: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,-8,prefix); break;
			case GKVoVRo: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,-16,prefix); break;
			...
			case GKPRqIb: if (prefix&GOT66) *cptr++ = xmmchar; i += getadst(&ibuf[i],cptr,-8,prefix); sprintf(&cptr[strlen(cptr)],", %d",(signed)ibuf[i]); i++; break;
			...
			case GKEdVd: j = getadst(&ibuf[i],cptr,max(opsize,4),prefix); sprintf(&cptr[strlen(cptr)],", %c%s",xmmchar,regmmnam[regind]); i += j; break;
			...
			case GKMdVd: break;
			case GKMoVo: j = getadst(&ibuf[i],cptr,-16,prefix); cptr += sprintf(&cptr[strlen(cptr)],", %c%s",xmmchar,regmmnam[regind]); i += j; break;
			case GKMqPq: j = getadst(&ibuf[i],cptr,-8,prefix); cptr += sprintf(&cptr[strlen(cptr)],", %s",regmmnam[regind]); i += j; break;
			case GKMqVq: cptr += sprintf(cptr,"qword ptr "); j = getadst(&ibuf[i],cptr,-16,prefix); cptr += sprintf(&cptr[strlen(cptr)],", %c%s",xmmchar,regmmnam[regind]); i += j; break;
			case GKMvGv: break;
			...
			case GKVdEd: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,-16,prefix); break;
			case GKVdWd: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,-16,prefix); break;
			case GKVdWq: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,max(opsize,4),prefix); break;
			case GKVoEd: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,max(opsize,4),prefix); break;
			case GKVoMo: break;
			case GKVoWo: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); if (prefix&GOTVEX) cptr += sprintf(cptr,"%cmm%d, ",xmmchar,rexreg); i += getadst(&ibuf[i],cptr,-16,prefix); break;
			case GKVoMq: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr, -8,prefix); break;
			case GKVoVo: break;
			case GKVoWd: break;
			case GKVoWq: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,-16,prefix); break;
			case GKVoWw: break;
			case GKVqEd: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,-16,prefix); break;
			case GKVqMq:      if ((!stricmp(xptr->st[y],"movlps")) && (ibuf[i] >= 192)) { strcpy(&cptr[-4],"hlps "); cptr++; } //Evil hack!
							 ...
							 cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,-8,prefix); break;
			case GKVqWd: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,-16,prefix); break;
			case GKVqWq: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,-16,prefix); break;
			case GKWdVd: j = getadst(&ibuf[i],cptr,-8,prefix); sprintf(&cptr[strlen(cptr)],", %c%s",xmmchar,regmmnam[regind]); i += j; break;
			case GKWoVo: j = getadst(&ibuf[i],cptr,-16,prefix); sprintf(&cptr[strlen(cptr)],", %c%s",xmmchar,regmmnam[regind]); i += j; break;
			case GKWqVq: j = getadst(&ibuf[i],cptr,-8,prefix); sprintf(&cptr[strlen(cptr)],", %c%s",xmmchar,regmmnam[regind]); i += j; break;
			case GKEdVoIb: break;
			...
			case GKVoMbIb: break;
			case GKVoMdIb: break;
			case GKVoMwIb: cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,max(opsize,4),prefix); sprintf(&cptr[strlen(cptr)],", %d",(signed)ibuf[i]); i++; break;
			case GKVoWdIb: break;
			case GKVoWqIb: break;

			case GKVdWdIb: j = getadst(&ibuf[i],cptr,-16,prefix); sprintf(&cptr[-5],"%sss %c%s, ",cmpnam[ibuf[i+j]&7],xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],&cptr[strlen(cptr)],-16,prefix); i++; break;
			case GKVqWqIb: j = getadst(&ibuf[i],cptr,-16,prefix); sprintf(&cptr[-5],"%ssd %c%s, ",cmpnam[ibuf[i+j]&7],xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],&cptr[strlen(cptr)],-16,prefix); i++; break;
			case GKVoWoIb:
				if (!memcmp(obuf,"cmpcc",5))
					  { j = getadst(&ibuf[i],cptr,-16,prefix); sprintf(&cptr[-5],"%sp%c %c%s, ",cmpnam[ibuf[i+j]&7],cptr[-2],xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],&cptr[strlen(cptr)],-16,prefix); }
				else { cptr += sprintf(cptr,"%c%s, ",xmmchar,regmmnam[regind]); i += getadst(&ibuf[i],cptr,-16,prefix); sprintf(&cptr[strlen(cptr)],", %d",(signed)ibuf[i]); }
				i++; break;
...
void main (int argc, char **argv)
{
	...
	while (memcmp(cptr,"\x90\x90\x90\x90\x90\x90\x90\x90",8))
	{
		...
		lincnt++; if (lincnt >= 64) { lincnt = 0; if (getch() == 27) exit(0); }
	}
